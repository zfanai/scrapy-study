...
class DmozSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        print 'response:', filename, response.url
        #filename='movie.html'
        filename=response.url.replace('http://', '').replace('/', '-')
        print 'filename:', filename
        with open(filename, 'wb') as f:
            f.write(response.body)
        #
        sel = response.xpath('//ul[@id="newPicList"]/li/a/@href')
        print 'sel:', sel, len(sel)
        for s in sel:
            href = s.extract()
            print 'href:', href
            yield scrapy.Request(self.host_url+href, callback=self.parse)

    def xx_parse(self, response):
        filename = response.url.split("/")[-2]
